...
class TextArea:
    def __init__(self, dimensions, image_dimensions, anchor_location):
        ...
        if (self.right > self.image_width) or (self.bottom > self.image_height):
            ...
            print(
                f"Text area Left={self.left}, Right={self.right}, Top={self.top}, Bottom={self.bottom}"
            )
...
class Word:
    ...
    def max_width_rectangle(self):
        return [
            (self.x - (self.max_width / 2), self.y - 10),
            (self.x + (self.max_width / 2), self.y + 10),
        ]
...
class WordGrid:
    ...
    def show_word_info(self):
        for w in self.words:
            print(
                f"{w.word_text} ---> (x,y) ({w.x},{w.y})  width={w.width} max_width={w.max_width}"
            )
    ...
    def calculate_grid_dimensions(self):
        ...
        if numwords > (self.rows * self.columns):
            print(
                f"This is only able to handle {self.MAX_ROWS * self.MAX_COLUMNS} words.  There were {numwords} submitted"
            )
            return None
    ...
    def calculate_word_positions(self):
        ...
        y = [
            self.text_area.top + y_offset * 0.50,
            self.text_area.top + y_offset * 1.5,
        ]  # adjust offset multipliers to adjust vertical spacing
    ...
    def draw_word_rectangles(self, draw):
        for word in self.words:
            ...
            draw.rectangle(
                [top_left, bottom_right], fill=(0, 0, 0), outline=(255, 0, 0), width=2
            )
            ...
            # The word is too long to fit in the expanded space
                ...
                while w.width == 0:
                    ...
                    if l1width > w.max_width or l2width > w.max_width:
                        ...
                        w.max_width = w.width
                        draw.text(
                            (w.x, w.y - 15), lines[0], fill, anchor="mm", font=font
                        )
                        draw.text(
                            (w.x, w.y + 15), lines[1], fill, anchor="mm", font=font
                        )
...
if __name__ == "__main__":
    ...
    word_strings = [
        ...
        "Clout",
        "TEST",
        "Long string of Words. what happens if its too long?",
        "orange",
        "blue",
    ]
