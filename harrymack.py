...
class Source:
    def __init__(self, base_name, path):
        ...
        # * these tracks should be split by searching for silence instead of a start and stop time
        self.chunks = ["WordplayWednesdayParts"]
...
def import_from_csv(csv_name):
    ...
    # ? need to create this list from the csv itself?  that way, to add (or delete) a column, I can just do it in excel and it will carry over here
    # ? but, that would eliminate the following check.  since the csv, by definition, would contain all of the headings that it should
